...
export const StaggeredMenu = ({
  position = 'right',
  colors = ['var(--color-ink)', 'var(--color-ink)'],
  items = [],
  socialItems = [],
  displaySocials = true,
  displayItemNumbering = true,
  className,
  logoUrl = '/safecut-logo.svg',
  ...
  useLayoutEffect(() => {
    ...
    return () => ctx.revert();
  }, [menuButtonColor, position, isReducedMotion]);
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (isReducedMotion) {
       ...
       if (numberEls.length) gsap.set(numberEls, { '--sm-num-opacity': 1 } as any);
  ...
  const animateColor = useCallback(
    ...
    [openMenuButtonColor, menuButtonColor, changeMenuColorOnOpen, isReducedMotion]
  );
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['var(--color-ink)', 'var(--color-ink)'];
          let arr = [...raw];
          if (arr.length >= 3) {
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
           {/* Replace with actual logo or just text if wanted */}
           <span className="font-display font-semibold text-lg" style={{ color: open ? openMenuButtonColor : menuButtonColor }}>SafeCut</span>
        </div>
        <button
          ref={toggleBtnRef}
          className="sm-toggle"
